...
export function RecordTableActionColumn({
  data,
  deleteStream,
}: RecordTableActionColumnProps) {
  ...
  const handleWatch = async () => {
    const { fileName, format, hashedFileName } = getFileInfo(
      data.outputFilePath,
      data.id.toString()
    );
  ...
  const handleDownload = async () => {
    const fileName =
      data.outputFilePath.split(/[/\\]/).pop() || `${data.id}.mp4`;
    const format = fileName.split(".").pop();
    ...
    try {
      // Download video file
      const videoProxyUrl = buildProxyUrl(
        `/files/${data.id}/${hashedFileName}`
      );
      ...
      const danmuExists = await checkFileExists(
        data.id.toString(),
        danmuFileName
      );

      if (danmuExists) {
        const danmuProxyUrl = buildProxyUrl(
          `/files/${data.id}/${danmuFileName}`
        );
