	...
	const populateForm = (type: ActionType): React.ReactNode => {
		...
		const forms = {
			...
			[ActionType.Rclone]: (
				<RcloneActionForm
					{...formProps}
					defaultValues={formProps.defaultValues as RcloneActionSchema}
					strings={{
			...
			[ActionType.Move]: (
				<MoveActionForm
					{...formProps}
					defaultValues={formProps.defaultValues as MoveActionSchema}
			...
			[ActionType.Copy]: (
				<CopyActionForm
					{...formProps}
					defaultValues={formProps.defaultValues as CopyActionSchema}
	...
	return (
		...
		>
			<AlertDialogTrigger asChild>
				<Button onClick={() => (open ? setOpen(false) : setOpen(true))} variant='outline'>
					{openIcon ?? strings.title}
				</Button>
			</AlertDialogTrigger>
			<AlertDialogContent className='max-h-[90%] overflow-auto sm:max-w-[425px] 3xl:max-h-[850px]'>
				<AlertDialogHeader>
					<AlertDialogTitle>{strings.title}</AlertDialogTitle>
					<AlertDialogDescription>{strings.description}</AlertDialogDescription>
				</AlertDialogHeader>
				...
				<p className={cn("text-[0.8rem] text-muted-foreground")}>{strings.actionTypeDescription}</p>
				<FormMessage />

				<div className='flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm'>
					<div className='space-y-0.5'>
						<Label>{strings.state}</Label>
						<p className={cn("text-[0.8rem] text-muted-foreground")}>{strings.stateDescription}</p>
					</div>
					<Switch checked={active} onCheckedChange={setActive} arial-label='State switch' />
				</div>
