...
export const RcloneActionForm = forwardRef<HTMLFormElement, RcloneActionFormProps>(
	({ defaultValues, strings, onSubmit }, ref) => {
		const form = useForm({
			resolver: async (data, context, options) => {
				console.log("formData", data)
				console.log("validation result", await zodResolver(rcloneActionSchema)(data, context, options))
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormDescription>{strings.operationDescription}</FormDescription>
									<FormMessage />
								</FormItem>
							)}
						/>
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormDescription>{strings.remotePathDescription}</FormDescription>
									<FormMessage />
								</FormItem>
							)}
						/>
						...
						{fields.length > 0 &&
							fields.map((field, index) => (
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel className={cn(index !== 0 && "sr-only")}>{strings.arguments}</FormLabel>
