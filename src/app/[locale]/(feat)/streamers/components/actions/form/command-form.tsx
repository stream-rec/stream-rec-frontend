...
export const CommandActionForm = forwardRef<HTMLFormElement, CommandActionFormProps>(
	({ defaultValues, strings, onSubmit }, ref) => {
		const form = useForm({
			resolver: async (data, context, options) => {
				console.log("formData", data)
				console.log("validation result", await zodResolver(commandActionSchema)(data, context, options))
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormDescription>{strings.programDescription}</FormDescription>
									<FormMessage />
								</FormItem>
							)}
						/>
