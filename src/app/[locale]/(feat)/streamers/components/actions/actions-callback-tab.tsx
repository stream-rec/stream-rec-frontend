	...
	return (
		<div className='mt-6 space-y-6'>
			<Alert>
				...
				<AlertDescription>{strings.alertDescription}</AlertDescription>
			</Alert>

			<div className={"space-y-6"}>
				<div className={"space-y-6"}>
					<div className={"flex flex-col items-center justify-between space-x-2 space-y-2 md:flex-row md:space-x-0"}>
						<div>
						...
						{list.map((field, index) => (
							<div key={"c-" + index}>
								<ActionCard
									...
									actionStrings={strings.actionStrings}
								/>
							</div>
						))}
					</div>
						...
						{endedList.map((field, index) => (
							<div key={"cx-" + index}>
								<ActionCard
									key={index}
									data={field}
									onDelete={() => deleteItemEnded(index)}
									onCheckedChange={value => {
										const updated = { ...field, enabled: value }
