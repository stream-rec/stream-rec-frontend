								...
								>
									<div className='space-y-0.5'>
										...
										<div className={"flex flex-row items-center gap-2"}>
											<Badge variant={result.isSuccess ? "secondary" : "destructive"}>
												{" "}
												{result.isSuccess ? t("success") : t("failed")}
											</Badge>
											{result.startTime != 0 && (
												<Badge variant={"secondary"}>{computeDate(new Date(result.startTime * 1000))}</Badge>
											)}
											{result.endTime != 0 && (
												<Badge variant={"secondary"}>{computeDate(new Date(result.endTime * 1000))}</Badge>
											)}
										</div>
									</div>
								</div>
