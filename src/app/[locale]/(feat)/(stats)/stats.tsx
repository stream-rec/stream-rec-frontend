...
export function CardsStats({ data, firstCard, secondCard }: CardDataProps) {
	...
	const formatDate = (date: number) => {
		...
		return month.toLocaleString(navigator.language, {
			month: "short",
			day: "numeric",
		})
	}
		...
		<div className='grid gap-4 sm:grid-cols-2 xl:grid-cols-2 xl:gap-x-16'>
			<Card>
				...
				<CardContent>
					...
					<p className='text-xs text-muted-foreground'>{firstCard.percentage}</p>
			...
			<Card>
				...
				<CardContent>
					...
					<p className='text-xs text-muted-foreground'>{secondCard.percentage}</p>
