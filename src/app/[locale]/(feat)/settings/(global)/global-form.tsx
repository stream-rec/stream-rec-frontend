...
import {
  ...
  formatBytes,
  formatSeconds,
} from "@/src/app/utils/conversions";
import {
  GlobalConfig,
  globalConfigSchema,
} from "@/src/lib/data/config/definitions";
...
export function GlobalForm({ appConfig, update, strings }: GlobalFormProps) {
  ...
  const [maxPartDurationFormat, setMaxPartDurationFormat] = useState(
  ...
  const formatAndSetValues = useCallback(
    (value: number, format: string, setFormat: Function, field: string) => {
  ...
  useEffect(() => {
    formatAndSetValues(
      appConfig.minPartSize,
      minPartSizeFormat,
      setMinPartSizeFormat,
      "minPartSize"
    );
    formatAndSetValues(
      appConfig.maxPartSize,
      maxPartSizeFormat,
      setMaxPartSizeFormat,
      "maxPartSize"
    );

    if (
      appConfig.maxPartDuration &&
      maxPartDurationFormat === DurationUnit.SECONDS
    ) {
  ...
  async function onSubmit(data: GlobalConfig) {
    data.minPartSize = data.minPartSize
      ? Math.round(convertToBytes(minPartSizeFormat, data.minPartSize))
      : data.minPartSize;
    data.maxPartSize = data.maxPartSize
      ? Math.round(convertToBytes(maxPartSizeFormat, data.maxPartSize))
      : data.maxPartSize;
    if (data.maxPartDuration) {
      if (data.maxPartDuration === 0) {
        data.maxPartDuration = undefined;
      } else {
        data.maxPartDuration = Math.round(
          convertToSeconds(maxPartDurationFormat, data.maxPartDuration)
        );
      }
    }
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        {engine === "kotlin" && (
          <KotlinEngineFields
            ...
            strings={{
              ...
              enableFlvDuplicateTagFilteringTitle:
                strings.enableFlvDuplicateTagFilteringTitle,
              enableFlvDuplicateTagFilteringDescription:
                strings.enableFlvDuplicateTagFilteringDescription,
        ...
        {(engine === "ffmpeg" || engine === "streamlink") && (
          <FfmpegBasedEngineFields
            form={form}
            strings={{
              useBuiltinSegmenterTitle: strings.useBuiltInSegmenter,
              useBuiltInSegmenterDescription:
                strings.useBuiltInSegmenterDescription,
            ...
            <FormItem>
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <SelectContent>
                    <SelectItem value={FileSizeUnit.B}>
                      {FileSizeUnit.B}
                    </SelectItem>
                    <SelectItem value={FileSizeUnit.KB}>
                      {FileSizeUnit.KB}
                    </SelectItem>
                    <SelectItem value={FileSizeUnit.MB}>
                      {FileSizeUnit.MB}
                    </SelectItem>
                    <SelectItem value={FileSizeUnit.GB}>
                      {FileSizeUnit.GB}
                    </SelectItem>
                  </SelectContent>
                </div>
                <FormDescription>{strings.minPartDescription}</FormDescription>
              </Select>
              <FormMessage />
            </FormItem>
          )}
          ...
          render={({ field }) => (
            <FormItem>
              ...
              >
                ...
                <FormDescription>{strings.maxPartDescription}</FormDescription>
              </Select>
              <FormMessage />
            </FormItem>
          )}
            ...
            <FormItem>
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <SelectContent>
                    <SelectItem value={DurationUnit.SECONDS}>
                      {strings.timeFormats.seconds}
                    </SelectItem>
                    <SelectItem value={DurationUnit.MINUTES}>
                      {strings.timeFormats.minutes}
                    </SelectItem>
                    <SelectItem value={DurationUnit.HOURS}>
                      {strings.timeFormats.hours}
                    </SelectItem>
                    <SelectItem value={DurationUnit.DAYS}>
                      {strings.timeFormats.days}
                    </SelectItem>
                  </SelectContent>
                </div>
                <FormDescription>
