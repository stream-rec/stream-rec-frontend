		...
		>
			{showSkipAds && (
				<FlagFormField
					...
					fieldName={"skipAds"}
					controlPrefix={controlPrefix}
					title={strings.skipAds}
					description={strings.skipAdsDescription}
				...
				render={({ field }) => (
					<FormItem>
						...
						<Select
							...
							allowNone={allowNone}
							options={(skipAds ? qualityOptions.slice(0, 1) : qualityOptions).map(quality => (
					...
					<FormItem>
						...
						<FormControl>
							<Input
								maxLength={30}
								minLength={30}
								placeholder={strings.authTokenPlaceholder}
								value={field.value}
								onChange={field.onChange}
							/>
						</FormControl>
						<FormDescription>{strings.authTokenDescription}</FormDescription>
						<FormMessage />
					</FormItem>
				)}
			/>
			...
			{showTtvProxyRelated && (
				<>
					<FlagFormField
						control={control}
						fieldName={"twitchProxyPlaylistFallback"}
						controlPrefix={controlPrefix}
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormDescription>{strings.ttvProxyPlaylistDescription}</FormDescription>
								<FormMessage />
							</FormItem>
						)}
					></FormField>
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormDescription>{strings.ttvProxyPlaylistExcludeDescription}</FormDescription>
								<FormMessage />
							</FormItem>
						)}
					></FormField>
